// --- app.js - Versi Final Lengkap Sebenarnya ---
...
function loadAllTransactions() {
    transactionCollection.orderBy('tanggal', 'asc').onSnapshot(snapshot => {
        ...
        entityListBody.innerHTML = `<tr><td colspan="3" class="text-center text-danger"><strong>Gagal memuat data.</strong><br><small>Pastikan Indeks Firestore sudah dibuat.</small></td></tr>`;
    });
}
...
function updateListTitle() {
    ...
    title.textContent = (currentDisplayMode === 'pembelian') 
...
function renderEntityList() {
    ...
    filteredTransactions.forEach(tx => {
        if (!entities[tx.nama]) {
            entities[tx.nama] = { balance: 0, name: tx.nama };
        }
        if (tx.type === 'faktur') {
            entities[tx.nama].balance += (tx.jumlah || 0) - (tx.retur || 0);
        } else if (tx.type === 'payment') {
            entities[tx.nama].balance -= (tx.jumlah || 0);
        }
    });
    ...
    Object.values(entities)
        ...
        .forEach(entity => {
            ...
            const row = `
                <tr>
                    <td><span class="fw-bold">${entity.name}</span></td>
                    <td class="text-danger">Rp ${formatCurrency(entity.balance)}</td>
                    <td>
                        <button class="btn btn-sm btn-info" onclick="openDetailsModal('${entity.name}')">
                            Detail
                        </button>
                    </td>
                </tr>`;
    ...
    if (!hasData) {
        entityListBody.innerHTML = `<tr><td colspan="3" class="text-center">Tidak ada data untuk ditampilkan.</td></tr>`;
    }
}
...
function updateDashboard() {
    ...
    allTransactions.forEach(tx => {
        ...
        if (tx.mode === 'pembelian' && tx.type === 'faktur') {
            totalPembelian += tx.jumlah || 0;
        } else if (tx.mode === 'penjualan' && tx.type === 'faktur') {
            totalPenjualan += tx.jumlah || 0;
        }
        if (tx.type === 'faktur' && new Date(tx.jatuhTempo) < new Date() && sisa > 0) {
            jatuhTempoCount++;
        }
    });
...
window.openDetailsModal = (entityName) => {
    const entityTransactions = allTransactions
        ...
        .sort((a, b) => (a.tanggal === b.tanggal) ? new Date(a.createdAt.seconds * 1000) - new Date(b.createdAt.seconds * 1000) : new Date(a.tanggal) - new Date(b.tanggal));
    ...
    entityTransactions.forEach(tx => {
        ...
        if (tx.type === 'faktur') {
            ...
            if (calculateSisaFaktur(tx.id) > 0) {
                actions += `<button class="btn btn-sm btn-success" onclick="openPaymentModal('${tx.id}', '${tx.noFaktur}')" title="Bayar Faktur Ini"><i class="bi bi-cash-coin"></i></button> `;
            }
...
function handlePaymentSubmit(e) {
    ...
    const paymentData = {
        mode: fakturAsli.mode,
        type: 'payment',
        linkedFakturId: fakturId,
        noFaktur: fakturAsli.noFaktur,
        nama: fakturAsli.nama,
...
function importFromExcel(e) {
    ...
    reader.onload = function(event) {
        try {
            ...
            batch.commit().then(() => {
                alert('Impor data faktur berhasil!');
                e.target.value = '';
            }).catch(err => {
